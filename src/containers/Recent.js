...
export default function Category(...props) {
  ...
  useEffect(() => {
    ...
    async function onLoad() {
      try {
        var articles = Cache.getItem("recent" + query);
        if (!articles) {
          articles = await loadArticles(query);
          Cache.setItem("recent" + query, articles);
        }
      ...
      setIsLoading(false);
      try {
        let cachedArticles = Cache.getItem("recent" + query);
        ...
        if (
          ...
          Cache.setItem("recent" + query, tempArticles);
        }
      } catch (e) {
        onError(e);
      }
